...
# # Import the analysis functions directly from agents (async)
...
#         # Run parser agents (async) on this *single* answer
#         humility_score, humility_evidence = await analyze_humility(answer)
...
#         self.analysis.turns.append(
#             InterviewTurn(question=question, answer=answer, analysis=analysis)
#         )
#         return analysis

#     def _generate_suggestions(self, scores: Dict[str, float]) -> List[str]:
#         out = []
#         if scores["humility"] < 6.0:
#             out.append("Use more 'we' language and explicitly acknowledge others’ contributions.")
#         if scores["learning"] < 6.0:
#             out.append("Describe specific learning loops: what you tried, what you changed, and results.")
#         if scores["feedback"] < 6.0:
#             out.append("Invite feedback explicitly (e.g., ‘What’s one thing I could improve?’).")
#         if scores["mistakes"] < 6.0:
#             out.append("Show ownership, RCA, and a prevention plan when discussing mistakes.")
#         return out or ["Solid overall responses. Keep highlighting collaboration, learning, and accountability."]

#     # def aggregate_scores(self) -> Dict[str, float]:
#     #     totals = {"humility": 0.0, "learning": 0.0, "feedback": 0.0, "mistakes": 0.0}
#     #     n = max(1, len(self.analysis.turns))
#     #     for t in self.analysis.turns:
#     #         for k in totals:
#     #             totals[k] += t.analysis["scores"][k]
#     #     for k in totals:
#     #         totals[k] = round(totals[k] / n, 1)
#     #     self.analysis.overall_scores = totals
#     #     return totals

#     def aggregate_scores(self) -> Dict[str, float]:
#         traits = ["humility", "learning", "feedback", "mistakes"]
#         buckets = {t: [] for t in traits}
#         for turn in self.analysis.turns:
#             for t in traits:
#                 buckets[t].append(float(turn.analysis["scores"][t]))

#         agg = {} 
#         for t in traits:
#             # Choose your method per trait if you want:
#             #   val = winsorized_mean(buckets[t], winsor=0.1)
#             #   val = median(buckets[t])
#             #   val = ema(buckets[t], beta=0.6)
#             val = trimmed_recency_weighted_mean(buckets[t], trim=0.15, decay=0.8)
#             agg[t] = round(val, 1)

#         self.analysis.overall_scores = agg
#         return agg

#     def aggregate_suggestions(self) -> List[str]:
#         # Aggregate trait‑level suggestions based on averages
#         avgs = self.aggregate_scores()
#         return self._generate_suggestions(avgs)

#     # These helpers let the report generator pull content
...
#                 "suggestions": t.analysis.get("suggestions", [])
#             })
#         return blocks
...
#             "summary_suggestions": self.aggregate_suggestions()
#         }
